...
>(
  (
    ...
    return (
      <div
        ref={ref}
        data-collapsible={finalCollapsible === 'icon' && state === 'collapsed'}
        className={cn(
            'hidden md:flex flex-col h-screen bg-sidebar text-sidebar-foreground border-r transition-all duration-300 ease-in-out',
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      data-sidebar="footer"
      className={cn("flex flex-col gap-2 p-4 border-t",
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant, size }), 
          isCollapsed && 'justify-center !p-2 !h-10 !w-10',
          className)}
        {...props}
      >
        {children}
